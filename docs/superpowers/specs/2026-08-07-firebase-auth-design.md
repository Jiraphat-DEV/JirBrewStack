...
- **ตั้ง persistence เองเป็น `browserLocalPersistence` ผ่าน `initializeAuth` ไม่ใช้ `getAuth()`** ข้อความเดิมในสเปกที่เขียนว่า default ของ Firebase Auth คือ `browserLocalPersistence` นั้นผิด ของจริง `getAuth()` initialize ด้วย chain `[indexedDBLocalPersistence, browserLocalPersistence, browserSessionPersistence]` แล้ว IndexedDB ชนะเสมอ ซึ่งพังกับ popup: คลาส IndexedDB persistence ตั้ง `isHiding` เป็น true ทุกครั้งที่ `document.visibilityState` เป็น `hidden` แล้ว `_openDb()` โยน `Error('Database is closing/hidden')` ตอน popup ของ Google ขึ้นมา หน้าหลักกลายเป็น hidden พอ popup กลับมา SDK เขียน user ลง DB ที่ปิดไปแล้ว `signInWithPopup` จึง reject ทั้งที่ OAuth ผ่านแล้ว และเพราะสิ่งที่โยนเป็น `Error` ธรรมดาไม่ใช่ `FirebaseError` จึงไม่มี `code` ให้จับด้วย เจอจริงบน Safari 2026-08-10 `browserLocalPersistence` เก็บบน `window.localStorage` ไม่มีวงจรเปิดปิดและไม่ผูกกับ visibility ยังอยู่ข้ามการรีเฟรชและข้ามการปิดเบราว์เซอร์ตาม req ข้อ 5 เหมือนเดิม `initializeAuth` ต้องส่ง `popupRedirectResolver: browserPopupRedirectResolver` เองด้วย ไม่งั้น `signInWithPopup` ใช้ไม่ได้
- **error ที่ไม่คาดคิดต้อง `console.error` เสมอ** เดิมโค้ดตั้งข้อความกลางๆ อย่างเดียวโดยไม่ log ตัว error พอ login พังจริงจึงไม่มีอะไรให้ไล่เลย ต้องใส่ log ชั่วคราวแล้วให้เจ้าของกดซ้ำถึงจะรู้สาเหตุ ข้อความที่ผู้ใช้เห็นยังเป็นข้อความกลางๆ เหมือนเดิม
