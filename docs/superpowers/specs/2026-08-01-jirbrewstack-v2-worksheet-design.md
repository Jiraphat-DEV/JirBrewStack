...
| `src/data/brew.js` | `computeRecipe`, `toMavo`, `buildTimerSteps`, `defaultPick` | ~120 บรรทัด |
| `src/data/brew.test.js` | `node --test` | ~230 บรรทัด |
...
- **input ที่เป็นไปไม่ได้**: ทุกช่องเป็นปุ่มจากรายการปิด และมีค่าตั้งต้นเสมอ จึงไม่มี state ที่ไม่ถูกต้อง `computeRecipe` โยน error ถ้าได้ key ที่ไม่รู้จัก หรือถ้า field ที่จำเป็นไม่มีค่าหลัง apply ครบทุกขั้น (เป็นบั๊กในไฟล์ rules ไม่ใช่ input ผู้ใช้) และมีเทสคุม
...
เทส 3, 8, 9, 10, 11, 12 และ 14 เป็นตัวคุมว่าถ้าใครไปแก้ตัวเลขใน `brewing-rules.js` แล้วทำหลักการพัง จะรู้ทันที ซึ่งเป็นจุดสำคัญเพราะไฟล์นั้นถูกออกแบบมาให้แก้บ่อย
